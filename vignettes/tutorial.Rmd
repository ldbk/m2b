...
observation of behaviour for a subset of the dataset.

This tutorial presents a new class named `xytb` and the functions that were
implemented for the use of this package, as well as an example of application.
...
The analytical steps are summarized in 4 main functions and methods. 

##`xytb`
`xytb` is a class, but also a method. 4 distinct signatures help the user to load 
...
During this step, 
derivated information on the movement data are computed and stored in the slots `dxyt`
and `befdxyt`. 

Three parameters can be set by the user: `winsize` specifies the 
sizes of sliding windows on which to compute the statistical operators, idquant'
specifies the quantiles to be computed, and 'move' (optional) specifies the
number of points for the calculated variables to be shifted backward. If included,
...
observation fromthe movement data. This is a simple wrapper calling the
randomForest function from the randomForest package
(\url{https://cran.r-project.org/web/packages/randomForest/index.html}), 
using the behavioural observations as a response, and the calculated movement
variables as predictors.

##`resRF` and `resB`
These two functions compute and plot the results related to the model (error
rate and convergence of the model `resRF`)
and the predicted behaviours (prediction vs observation over time or space, `resB`).

##`xytb2ltraj` and `ltraj2xytb`

These functions import or export an `xytb` object to the `ltraj` class. The
latter is used in the `adehabitatLT` package and numerous function
dedicated to the analysis of trajectories (see @calenge2008).

##`xytb2hmm` 

This function import an `xytb` object to the `moveHMM` class. The
latter is used in the `moveHMM` package and provide function
...
be observed. This parameters has to be passed to the functions during 
the analysis: the parameter `nob` is equal to '-1' in our case (see functions' help). 
...
of the model.
The function `extractRF` made the model available in the `randomForest`
